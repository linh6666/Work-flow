...
interface CreateJobModalProps {
  ...
  onClose: () => void;
  defaultDept?: string;
  onSave: (job: { dept: string; name: string }) => void;
}

export default function CreateJobModal({ isOpen, onClose, defaultDept = 'Điện', onSave }: CreateJobModalProps) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-xs animate-in fade-in duration-200">
      ...
      <div className="relative bg-white w-full max-w-[390px] rounded-2xl shadow-lg border border-slate-100/50 overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col p-6">
        ...
        <div className="flex items-center justify-between mb-5">
          <h3 className="text-base font-bold text-slate-800">Thêm hạng mục công việc</h3>
          <button 
            type="button"
            onClick={onClose}

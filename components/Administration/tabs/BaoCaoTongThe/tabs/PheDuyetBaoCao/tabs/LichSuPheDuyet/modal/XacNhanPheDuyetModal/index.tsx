...
export interface ReportItemForApproval {
  id: string;
  project: string;
  department: string;
  taskName: string;
...
interface XacNhanPheDuyetModalProps {
  isOpen: boolean;
  onClose: () => void;
  report: ReportItemForApproval | null;
  initialMode?: 'Đã duyệt' | 'Từ chối';
  onConfirm: (id: string, newStatus: 'Đã duyệt' | 'Từ chối', reason?: string) => void;
}

export default function XacNhanPheDuyetModal({
  isOpen,
  onClose,
  report,
  initialMode = 'Đã duyệt',
  onConfirm,
}: XacNhanPheDuyetModalProps) {
  const [selectedAction, setSelectedAction] = useState<'Đã duyệt' | 'Từ chối'>('Đã duyệt');
  const [reason, setReason] = useState('');

  useEffect(() => {
    if (isOpen) {
      setSelectedAction(initialMode);
      setReason('');
    }
  }, [isOpen, initialMode]);
  ...
  const handleConfirm = () => {
    onConfirm(report.id, selectedAction, reason);
    onClose();
  };
      ...
      <div className="bg-white rounded-2xl shadow-2xl border border-slate-100 w-full max-w-md overflow-hidden text-left transform transition-all">
        ...
        <div className="p-5 pb-3 flex items-start justify-between gap-3">
          <h3 className="text-sm font-bold text-slate-800 leading-snug">
            {selectedAction === 'Đã duyệt' ? 'Xác nhận duyệt — ' : 'Từ chối báo cáo — '}
            <span className="font-extrabold text-slate-900">{report.taskName}</span>
          </h3>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="px-5 py-3 space-y-4">
          ...
          {/* REJECTION REASON AREA (IF TỪ CHỐI SELECTED) */}
          {selectedAction === 'Từ chối' && (

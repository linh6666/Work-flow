...
interface BoQuaViPhamModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export default function BoQuaViPhamModal({
  isOpen,
  onClose,
  item,
  onConfirm,
}: BoQuaViPhamModalProps) {
        ...
        <form onSubmit={handleConfirm} className="p-4 sm:p-6 space-y-4 overflow-y-auto flex-1">
          ...
          <div className="space-y-1.5">
            <label className="block text-xs font-bold text-slate-800">
              Lý do bỏ qua / khóa vi phạm <span className="text-slate-400 font-normal">(bắt buộc)</span>
            </label>
            <textarea
              rows={4}
              value={reason}
              onChange={(e) => setReason(e.target.value)}
              placeholder="VD: Trường hợp ngoại lệ được Ban Giám đốc chấp thuận điều chỉnh kế hoạch..."
              className="w-full px-3.5 py-3 border-2 border-amber-500 rounded-xl text-xs text-slate-800 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-amber-500/20 shadow-2xs resize-none transition-all"
              required
            />
          </div>
          ...
          <div className="flex items-center justify-end gap-2.5 pt-2">
            ...
            <button
              ...
              className="px-4 py-2 bg-amber-600 hover:bg-amber-700 disabled:opacity-50 disabled:cursor-not-allowed text-white font-bold rounded-lg text-xs flex items-center gap-1.5 shadow-sm transition-all cursor-pointer"
            >

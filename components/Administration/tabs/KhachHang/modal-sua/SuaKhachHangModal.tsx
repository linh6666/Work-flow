...
interface SuaKhachHangModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (khachHang: KhachHangItem) => void;
  customer: KhachHangItem | null;
}

export default function SuaKhachHangModal({
  isOpen,
  onClose,
  onSave,
  customer,
}: SuaKhachHangModalProps) {
  ...
  // Reset form values when modal opens or customer changes
  useEffect(() => {
    if (isOpen && customer) {
      setMa(customer.ma || '');
      setLoai(customer.loai || 'Tiềm năng');
      setTen(customer.ten || '');
      setDaiDienHoTen(customer.daiDienHoTen || '');
      setDaiDienChucDanh(customer.daiDienChucDanh || '');
      setNguoiLienHe(customer.nguoiLienHe || '');
      setDienThoai(customer.dienThoai || '');
      setEmail(customer.email || '');
      setDiaChi(customer.diaChi || '');
      setMaSoThue(customer.maSoThue || '');
      setNguonKhachHang(customer.nguonKhachHang || '');
      setSoTaiKhoan(customer.soTaiKhoan || '');
      setNganHang(customer.nganHang || '');
    ...
    <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center z-50 animate-fade-in p-4">
      ...
      <div className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl border border-slate-100 overflow-hidden flex flex-col max-h-[90vh] transform transition-all scale-100 animate-scale-up">
        ...
        <div className="px-6 py-5 flex items-center justify-between border-b border-slate-100 shrink-0">
          <h3 className="text-xl font-bold text-slate-800">Chỉnh sửa thông tin khách hàng</h3>
          <button 
            type="button"
            onClick={onClose}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input 
                  ...
                  className="w-full text-sm bg-white border border-slate-200 rounded-lg px-3.5 py-2.5 text-slate-800 font-medium focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500"
                />
              </div>

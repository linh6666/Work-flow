...
export default function DeleteStaffModal({ isOpen, onClose, staff, deptName, onConfirm }: DeleteStaffModalProps) {
      ...
      <div className="relative bg-white w-full max-w-[390px] rounded-2xl shadow-lg border border-slate-100/50 overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col p-6">
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-base font-bold text-slate-800">Xác nhận xóa nhân viên</h3>
          </div>
          <button 
            type="button"
            onClick={onClose}
        ...
        <div className="mb-6">
          <p className="text-xs text-slate-500 leading-relaxed">
            Bạn có chắc chắn muốn xóa nhân viên <strong className="text-slate-800 font-bold">{staff.name}</strong> ({staff.role}) khỏi danh sách mặc định của <strong className="text-slate-800 font-bold">{deptName}</strong> không?
          </p>
          <p className="text-[11px] text-red-500 mt-2 font-semibold bg-red-50/50 px-2.5 py-1.5 rounded-lg border border-red-100/40">
            Hành động này sẽ xóa nhân viên ra khỏi danh sách cấu hình và không thể hoàn tác.
          </p>
        </div>
